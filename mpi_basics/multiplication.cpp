...
int main(int argc, char** argv)
{
  ...
  int row { rank / 5 }, column { rank % 5 };

  std::array<double, 24> local {};
  for (int j {}; j < 6; ++j) local[row * 6 + j] = A_element * B[column * 6 + j];

  std::array<double, 24> C {};
  MPI_Reduce(local.data(), C.data(), local.size(), MPI_DOUBLE, MPI_SUM, 0,
             MPI_COMM_WORLD);
